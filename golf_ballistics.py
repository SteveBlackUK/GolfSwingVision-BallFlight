# https://github.com/cagrell/golfmodel
from scipy.integrate import odeint
from itertools import groupby
import pandas as pd
import numpy as np

class golf_ballistics():
    """
    Based on:
    ...
    def initiate_hit(self, velocity, launch_angle_deg, off_center_angle_deg, 
        ...
        spin_axis_angle_deg = np.arctan2(sidespin_rpm, backspin_rpm) * 180 / np.pi



        self.spin = total_spin_rpm/60 # revolutions per second
        self.spin_angle = spin_axis_angle_deg/180*np.pi
...
#     # Create an instance of the golf_ballstics class
#     ball = golf_ballstics()

#     # Define input parameters
#     #take input form user in miles per hour and convert to m/s
#     velocityMPH = 112.7
#     velocity = velocityMPH*0.44704
#     launch_angle_deg = 18.8  # degrees
#     off_center_angle_deg = 0.5  # degrees
#     backspin_rpm = 4649  # revolutions per minute
#     sidespin_rpm = -2198  # rpm
#     windspeed = 0  # m/s
#     windheading_deg = 180  # degrees (tailwind)
...
#     # x, y = ball.get_landingpos(velocity, launch_angle_deg, off_center_angle_deg, 
#     #                 spin_rpm, spin_angle_deg, windspeed, windheading_deg)

#     # Print the landing position
#     # convert x and y from meters to yards before returning
#     print(f"Landing position (x, y): ({x*1.09361:.2f}, {y*1.09361:.2f})")
#     # print(f"Landing position (x, y): ({x:.2f}, {y:.2f})")
    
#     # Retrieve the maximum z-value from the simulation results
#     max_z = ball.df_simres['z'].max()

#     # Print the maximum z-value
#     # convert z from meters to yards before returning
#     print(f"Maximum z-value: {max_z*1.09361:.2f} yards")
#     # print(f"Maximum z-value: {max_z:.2f} meters")